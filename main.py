...
helix = twitch.Helix('','')
bot = TwitchBot.Client(app_api=helix, user_api=('', ''))

channel = helix.user('jesusavgn')
...
channel_emojis = Emojis.ChannelEmojis(auth=bot.auth, channel='jesusavgn').as_names()
global_bttv = ThirdpartyEmojis.GlobalBTTV().as_names()
channel_bttv = ThirdpartyEmojis.BTTV(auth=bot.auth, channel='jesusavgn').as_names()
global_ffz = ThirdpartyEmojis.GlobalFFZ().as_names()
channel_ffz = ThirdpartyEmojis.FFZ(auth=bot.auth, channel='jesusavgn').as_names()

# Getting user nicknames and messages text for word clouds
exclude_users = ['moobot', 'oldboty', 'kryabot']
...
wordcloud_users = WordClouds.ColoredCloud(' '.join(users_activity), image=Image.open('examples/hesus-base-2.png'), font='examples/arial.ttf', max_font_size=50)
wordcloud_words = WordClouds.ColoredCloud(' '.join(words_activity), image=Image.open('examples/hesus-base-2.png'), font='examples/arial.ttf', max_font_size=50)

wordcloud_users.as_png('generated/', 'hesus-users')
wordcloud_words.as_png('generated/', 'hesus-words')
